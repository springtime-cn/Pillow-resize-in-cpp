...
#ifndef PILLOW_RESIZE_PALETTE
#define PILLOW_RESIZE_PALETTE
...
ImagingPaletteNew(const char* mode)
{
    ...
    palette = (ImagingPaletteInstance*)calloc(1, sizeof(struct ImagingPaletteInstance));
    if (!palette) {
...
ImagingPaletteDuplicate(ImagingPalette palette)
{
    ...
    new_palette = (ImagingPaletteInstance*)malloc(sizeof(struct ImagingPaletteInstance));
    if (!new_palette) {
        return (ImagingPalette) ImagingError_MemoryError();
    }
