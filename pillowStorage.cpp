...
#ifndef PILLOW_RESIZE_STORAGE
#define PILLOW_RESIZE_STORAGE
...
ImagingMemoryArenaStruct ImagingDefaultArena = {
    1,                   // alignment
...
ImagingMemorySetBlocksMax(ImagingMemoryArena arena, int blocks_max)
{
    ...
    if (blocks_max == 0 && arena->blocks_pool != NULL) {
        ...
        arena->blocks_pool = (ImagingMemoryBlock *)p;
    } else {
        arena->blocks_pool = (ImagingMemoryBlock *)calloc(sizeof(*arena->blocks_pool), blocks_max);
...
memory_get_block(ImagingMemoryArena arena, int requested_size, int dirty)
{
    ...
    if (arena->blocks_cached > 0) {
        ...
        if (block.size != requested_size){
            block.ptr = (char*)realloc(block.ptr, requested_size);
        }
        if ( ! block.ptr) {
        ...
        if (dirty) {
            block.ptr = (char*)malloc(requested_size);
        } else {
            block.ptr = (char*)calloc(1, requested_size);
        }
...
memory_return_block(ImagingMemoryArena arena, ImagingMemoryBlock block)
{
    if (arena->blocks_cached < arena->blocks_max)  {
        ...
        if (block.size > arena->block_size) {
            ...
            block.ptr = (char*)realloc(block.ptr, arena->block_size);
        }
...
ImagingAllocateArray(Imaging im, int dirty, int block_size)
{
    ...
    im->blocks = (ImagingMemoryBlock *)calloc(sizeof(*im->blocks), blocks_count + 1);
    if ( ! im->blocks) {
        return (Imaging) ImagingError_MemoryError();
    }
 ...
 * now support mode are: "L", "RGB", "RGBA"
 * *******************************************************/
Imaging cvMatToImaging(const cv::Mat &imgMat, const char* mode){
    if(!mode){
        std::cout << "You have to assign the mode." << std::endl;
        return NULL;
    }
    if(!(strcmp(mode,"L")==0 || strcmp(mode,"RGB")==0 
        || strcmp(mode,"RGBA")==0)){
        std::cout << "Your assigned mode is not supported now." << std::endl;
        return NULL;
    }

    Imaging img = ImagingNewBlock(mode, imgMat.cols, imgMat.rows);
    if(!img)
        return NULL;
    for(int y=0; y<imgMat.rows; ++y){
        for(int x=0; x<imgMat.cols; ++x){
            if(strcmp(mode, "L") == 0)
                img->image[y][x] = imgMat.at<uchar>(y, x);
            else if(strcmp(mode, "RGBA") == 0){
                for(int k=0; k<4; ++k)
                    img->image[y][x*4+k] = imgMat.at<cv::Vec4b>(y, x)[k];
            }
            else if(strcmp(mode, "RGB") == 0){
                for(int k=0; k<3; ++k)
                    img->image[y][x*4+k] = imgMat.at<cv::Vec3b>(y, x)[k];
            }
            else{
                std::cout << "Your assigned mode is not supported now." << std::endl;
                return NULL;
...
int ImagingTocvMat(Imaging img, cv::Mat &imgMat){
    ...
    const char* mode = img->mode;
    if(strcmp(mode,"L")==0)
        imgMat.create(img->ysize, img->xsize, CV_8UC1);
    else if(strcmp(mode, "RGBA") == 0)
        imgMat.create(img->ysize, img->xsize, CV_8UC4);
    else if(strcmp(mode, "RGB") == 0)
        imgMat.create(img->ysize, img->xsize, CV_8UC3);
    else{
        std::cout << "This img mode is not supported." << std::endl;
        return -1;
    }
    for(int y=0; y<imgMat.rows; ++y){
        for(int x=0; x<imgMat.cols; ++x){
            if(strcmp(mode, "L") == 0)
                imgMat.at<uchar>(y, x) = img->image[y][x];
            else if(strcmp(mode, "RGBA") == 0)
                for(int k=0; k<4; ++k)
                    imgMat.at<cv::Vec4b>(y, x)[k] = img->image[y][x*4+k];
            else if(strcmp(mode, "RGB") == 0){
                for(int k=0; k<3; ++k)
                    imgMat.at<cv::Vec3b>(y, x)[k] = img->image[y][x*4+k];
            }
            else{
                std::cout << "This img mode is not supported." << std::endl;
                return -1;
 ...
 * *******************************************************/
int pillowResize(cv::Mat &imgIn, cv::Mat &imgOut, const char* mode, int width, int height, int filter){
    if(imgIn.empty()){
        std::cout << "Your input img is empty." << std::endl;
        return -1;
    }
    if(width<=0 || height<=0){
        std::cout << "width or height must be big than zero." << std::endl;
        return -1;
    }
    if(!(filter==IMAGING_TRANSFORM_NEAREST || 
        filter==IMAGING_TRANSFORM_LANCZOS || 
        filter==IMAGING_TRANSFORM_BILINEAR || 
        filter==IMAGING_TRANSFORM_BICUBIC ||
        filter==IMAGING_TRANSFORM_BOX || 
        filter==IMAGING_TRANSFORM_HAMMING)){
        imgOut.release();
        std::cout << "Check the giving filter is right?" << std::endl;
        return -1;
    }
    Imaging img = cvMatToImaging(imgIn, mode);
    float box[4] = {0,0,(float)img->xsize,(float)img->ysize};
    Imaging imgResize = NULL;
    if(filter==IMAGING_TRANSFORM_NEAREST){
        double a[6];
        memset(a, 0, sizeof a);
        a[0] = (double) (box[2] - box[0]) / width;
        a[4] = (double) (box[3] - box[1]) / height;
        a[2] = box[0];
        a[5] = box[1];
        imgResize = ImagingNewDirty(mode, width, height);
        imgResize = ImagingTransform(
            imgResize, img, IMAGING_TRANSFORM_AFFINE,
            0, 0, width, height,
            a, filter, 1);
    }
    else if(filter==IMAGING_TRANSFORM_LANCZOS) 
        imgResize = ImagingResample(img, width, height, IMAGING_TRANSFORM_LANCZOS, box);
    else if(filter==IMAGING_TRANSFORM_BILINEAR) 
        imgResize = ImagingResample(img, width, height, IMAGING_TRANSFORM_BILINEAR, box);
    else if(filter==IMAGING_TRANSFORM_BICUBIC) 
        imgResize = ImagingResample(img, width, height, IMAGING_TRANSFORM_BICUBIC, box);
    else if(filter==IMAGING_TRANSFORM_BOX) 
        imgResize = ImagingResample(img, width, height, IMAGING_TRANSFORM_BOX, box);
    else if(filter==IMAGING_TRANSFORM_HAMMING) 
        imgResize = ImagingResample(img, width, height, IMAGING_TRANSFORM_HAMMING , box);
    else{
        std::cout << "Check the giving filter is right?" << std::endl;
        imgOut.release();
        return -1;
    }
    ImagingTocvMat(imgResize, imgOut);
    ImagingDelete(img);
    ImagingDelete(imgResize);
    return 0;
}
